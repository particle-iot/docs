...
Each pin on the Core can either be configured as input (with or without pull-up or pull-down) or as output (push-pull or open-drain) using the [pinMode()](/cards/firmware/input-output/pinmode/) function.


After setting them up, the user can then write to or read from the pins using [digitalWrite()](/cards/firmware/input-output/digitalwrite/) and [digitalRead()](/cards/firmware/input-output/digitalread/) functions respectively.
...
Pins A0 to A7 can be set up as analog inputs and can measure voltages of up to 3.3V and are internally referenced to VDD. The user can read the pins using [analogRead()](/cards/firmware/input-output/analogread-adc/) function which returns a 12bit value.
...
Having said that, the user can send analog values to the pins using the function [analogWrite().](/cards/firmware/input-output/analogwrite-pwm/)
...
Configuration and use of all of these serial ports is described in the [serial functions.](/cards/firmware/serial/serial/)
