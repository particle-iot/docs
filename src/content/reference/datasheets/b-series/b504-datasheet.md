---
title: B-Series B504 datasheet
layout: commonTwo.hbs
columns: two
description: Datasheet for the Particle B-Series B504 SoM, Gen 3 cellular LTE Cat 1 Americas
---
...
The B504 is certified with the following cellular antenna:

| Antenna | SKU | Details | Links |
| :----- | :--- | :------ | :---- |
| Wide band LTE cell antenna [x1] | PARANTCW1EA | B504 and M-SoM | [Datasheet](/assets/pdfs/PARANTCW1EA.pdf) |
...
There are some optional B504 module specific I/O:
...
The B504 module has 4 pads at the bottom exposing the SWD interface of the nRF52840. This interface can be used to debug your code or reprogram your B504 bootloader, device OS, or the user firmware. We use 4 pogo-pins connecting to these pads during production for firmware flashing.
...
The B504 supports 8 ADC inputs.
...
The B504 supports one UART serial interfaces. 
...
The B504 supports two SPI (serial peripheral interconnect) ports.
...
The B504 supports two I2C (two-wire serial interface) ports.
...
The B504 supports PWM (pulse-width modulation) on the following pins:
...
The B504 supports a USB interface for programming the device and for USB serial (CDC) communications. The module itself does not contain a USB connector; you typically add a micro USB or USB C connector on your base board. It is optional but recommended.
...
The B504 supports an external common anode RGB LED. 
...
<sup>4</sup>RI is available on the B504 (Quectel) but not on the B402 (u-blox LTE M1)

<sup>5</sup>The VCC maximum is 4.3V on the B504 (Quectel) but is 4.2V on the B402 (u-blox LTE M1). For compatibility across modules, limit this to 4.2V.
...
Unlike the Boron, the B504 module does not have an on-module RGB system status LED. We have provided its individual control pins for you to connect an LED of your liking. This will allow greater flexibility in the end design of your products.
...
The 3D models are the same for the B504 and B523 as the cellular modem module is the same size.
