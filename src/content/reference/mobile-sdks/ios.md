...
- If you **want to contribute**, submit a pull request. Also be sure to check out [the contribution guidelines](https://particle-iot.github.io/#contributions).
...
Please follow the procedure described [in our guide](/reference/cloud-apis/authentication/) to create those strings,
...
If you set `productMode ` to `YES` / `true` be sure to also provide the `productId` (and `productName`) - please [read here](/getting-started/console/console/#your-product-id) about how to find your productId number.

Make sure you inject the `ParticleCloud` class with [scoped OAuth credentials for creating customers](/reference/cloud-apis/authentication/), so app users could create an account. [Read here](/reference/mobile-sdks/ios/#oauth-client-configuration) on how to do it correctly.
...
method will be called, if `(result == ParticleSetupMainControllerResultSuccess)` or (or simply `(result == .Success)` in Swift) the device parameter will contain an active `ParticleDevice` instance you can interact with using the iOS Cloud SDK.
