...
First, get the [Particle CLI](/photon/cli). Unless someone has already set it up for you, you're going to need it to connect your device. 
...
Otherwise, follow the instructions on how to [connect over USB](/photon/connect/#connecting-your-device-connect-over-usb) **BUT BEFORE YOU DO THAT** please read on!
...
Now that you've read that, go ahead and [connect over USB](/photon/connect/#connecting-your-device-connect-over-usb). When that's all done, you can go on to the next section.
...
Particle Dev has a lot of wonderful features. Namely, you can monitor cloud variables, cloud functions, and serial output from your devices via the Devices part of the menu bar. More on that in the [next section](/photon/hackathon/#now-do-stuff).
...
If you're 100% sure you did it right but it's still not working, send us an email at hello@particle.io and go ahead and try it with the [CLI tools](/photon/connect/#connecting-your-device-connect-over-usb).
...
Building a mobile app to hook up with a Particle device? Check out our [Mobile SDK](http://docs.particle.io/photon/mobile/).
...
If you go to the Particle menu you will also see `Show cloud variables`. This will allow you to see any variables you register with [Spark.variable](/photon/firmware/#spark-variable).

You can also monitor any [serial output](/photon/firmware/#communication-serial) with the `Show serial monitor` command. Make sure the proper device is selected through `Particle` > `Select device` when you do this.
...
Now you can compile and flash your first `.ino` file from Particle Dev. You are welcome to take any of [these examples](http://docs.particle.io/photon/examples/) as a first try and flash it to your device by clicking the lightning bolt in the upper right corner. The examples explain how Variable, Function, Publish, and Subscribe are used, but documentation on coding these cloud-exposed elements is available [here](http://docs.particle.io/photon/firmware/#cloud-functions). Check out [Tips and Tricks](/photon/hackathon/#tips-and-tricks) for some of the subtleties of coding these functions.
...
In the [quintessential example](/photon/firmware/#spark-function) for Spark Function, we are calling a brewCoffee function with the argument "coffee". This is nice if we have an array of choices we want to identify with strings, but what if we want to pass `int` data to our `Spark.function`?
