...
In Device OS 6.2.0 and later, binary data can be published by devices efficiently and delivered to webhooks.
...
The binary data is sent in binary format over the air (in the CoAP packet), so it does not
...
size, however, and the size after Base 64 encoding can exceed the 1k or 16K over-the-air limits.
...
For more information, see [Typed and extended publish](/reference/device-os/typed-publish/).


### Structured binary data

In Device OS 6.3.0 and later, structured binary data is supported. For more information, see [Typed and extended publish](/reference/device-os/typed-publish/).

For example, you could use this code on-device:
...
The hex data in `b` is efficiently re-encoded in binary CBOR format over-the-wire instead of including a text-based format like hex, Base 64, or Base 85.

When the event is received by a webhook, it would receive this data. The `_data` is Base 64 encoded, and this occurs in the cloud side, so the over-the-air data size limit
applies to the binary size, before encoding.
