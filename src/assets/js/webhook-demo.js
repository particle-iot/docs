$(document).ready(function() {
    ...
    const updateExplainer = function(explainObj) {
        ...
        $(flexContainerElem).css('display', 'flex');

        {
            const flexItemElem = document.createElement('div');
            $(flexItemElem).css('flex-basis', explainerSettings.eventWidth);
            

            if (explainObj.kind == 'event') {
                let text = '';
                if (explainObj.event.name == webhookName) {
                    text += 'This is the Particle event ';
                ...
                if (explainObj.event.name.indexOf('hook-sent') >= 0) {
                    text += 'This indicates that the Particle webhook server has sent the event to your webhook server'
                }
                else
                if (explainObj.event.name.indexOf('hook-response') >= 0) {
                    text += 'This is the response data from your webhook server, converted into a Particle event that can be received by your device.'
                ...
                $(flexItemElem).append(tableElem);
            }
            $(flexContainerElem).append(flexItemElem);
        }
        {
            const flexItemElem = document.createElement('div');
        ...
        $('#explainerTable').append(flexContainerElem);
