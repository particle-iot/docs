...
const familyMapCreate = function() {
    ...
    familyMap.drawMap = async function() {
        ...
        */
        ...
        // A group object may specify a mapColor (index into colors), a mapHatch
        // ('forward'/'backward'/'cross'), both, or neither. NO_COLOR_INDEX is used
        // when a country only picks up a hatch (from a group with no mapColor of its
        // own) so it still renders as a hatch over the default fill color, and can
        // combine with a mapColor contributed by a different group for that country.
        const NO_COLOR_INDEX = colors.length;
        const { styles, getStyleIndex } = worldMapGlobal.createHatchStyles([...colors, undefined]);
        ...
        datastore.data.countries.forEach(function(countryObj) {
            ...
            datastore.data.countryModemSim.forEach(function(cmsObj) {
                ...
                models.forEach(function(modelObj) {
                    if (modelObj.sim != cmsObj.sim || modelObj.modem != cmsObj.modem) {
                        return;
                    }

                    // A model with mapShowPOSS treats NR and POSS the same as YES for map purposes
                    let recommended = cmsObj.recommendation == 'YES';
                    if (!recommended && modelObj.mapShowPOSS && (cmsObj.recommendation == 'NS' || cmsObj.recommendation == 'POSS')) {
                    ...
                    if (typeof modelObj['mapColor'] != 'undefined') {
                        foundColorIndex = modelObj.mapColor;
                    }
                    if (modelObj.mapHatch) {
                        foundHatch = modelObj.mapHatch;
                    }
                });
            });
            if (typeof foundColorIndex != 'undefined' || foundHatch) {
                const colorIndex = typeof foundColorIndex != 'undefined' ? foundColorIndex : NO_COLOR_INDEX;
                worldMapInstance.setCountryColor(countryObj.isoCode, getStyleIndex(colorIndex, foundHatch));
            }
        });
        ...
        //
        {
            // CSS approximations of the SVG hatch patterns generated by worldMapGlobal
            // for the map itself (see world-map.js generatePatternElem)
            const hatchBackgroundImages = {
                backward: 'repeating-linear-gradient(45deg, #000000, #000000 2px, transparent 2px, transparent 15px)',
                forward: 'repeating-linear-gradient(135deg, #000000, #000000 2px, transparent 2px, transparent 15px)',
                cross: 'repeating-linear-gradient(45deg, #000000, #000000 2px, transparent 2px, transparent 15px), repeating-linear-gradient(135deg, #000000, #000000 2px, transparent 2px, transparent 15px)',
            };
            // Fallback swatch background for a group with a mapHatch but no mapColor of its own,
            // matching the map's default fill color.
            const defaultFillColor = worldMapGlobal.colorNames[worldMapGlobal.optionDefaults.fillColor];

            let html = '<div><table><thead><tr><th></th><th>SKU</th><th>Description</th><th>Lifecycle</th></tr></thead><tbody>';

            models.forEach(function(skuFamilyObj) {
                const hasMapColor = typeof skuFamilyObj['mapColor'] != 'undefined' && colors[skuFamilyObj.mapColor];
                const hatchBackgroundImage = hatchBackgroundImages[skuFamilyObj.mapHatch];
                if (!hasMapColor && !hatchBackgroundImage) {
                    return;
                }
                let style = 'background-color:' + (hasMapColor ? colors[skuFamilyObj.mapColor] : defaultFillColor);
                if (hatchBackgroundImage) {
                    style += ';background-image: ' + hatchBackgroundImage;
                }
