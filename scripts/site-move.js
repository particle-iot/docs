...
function preflight1() {
	...
	if (sourceLayout == '') {
		console.log(chalk.red('Error!') + ' Path does not specify a known layout, can\'t be moved');
		done();
		return;			
	}
	...
	if (sourceLayout != destLayout) {
		console.log('- will change layout from ' + sourceLayout + ' to ' + destLayout);
		checkLayout({sourceMd:sourceMd, sourceLayout:sourceLayout, destLayout:destLayout});
	}
	
	checkRedirects({fromUrl:mdPathToUrl(sourceMd), toUrl:mdPathToUrl(destMd)});
...
function checkLayout(options) {
	...
	var replacementData = textData.replace(/layout *: *[^\n]+/, 'layout: ' + options.destLayout);
...
function checkRedirects(options) {
	...
	for(var key in jsonData) {
		if (jsonData[key] == options.fromUrl) {
			console.log('- ' + key + ' was redirected to ' + options.fromUrl + ' changing to ' + options.toUrl);
	...
	while(true) {
		...
		if (end >= 0) {
			...
			if (href.startsWith(options.fromUrl)) {				
				...
				md = md.substring(0, possible + 2) + options.toUrl + md.substring(possible + 2 + options.fromUrl.length);
				updated = true;
