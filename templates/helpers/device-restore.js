        ...
        info.platforms.forEach(function(platformObj) {
            ...
            if (mode == 'zip') {
                if (info.versionsZip[version] && info.versionsZip[version].includes(platformObj.name)) {
                    const script = "gtag('send', 'event', 'Download', 'Device Restore Zip Download', '" + version + "/" + platformObj.name + "'); return true;";
                ...
                if (info.versions[version].includes(platformObj.name)) {
                    if (mode == 'download') {
                        const script = "gtag('send', 'event', 'Download', 'Device Restore Hex Download', '" + version + "/" + platformObj.name + "'); return true;";
